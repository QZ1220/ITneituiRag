...
Milvus过滤表达式语法规则：
1. 字符串字段需要用双引号包围，如: title == "成都招聘"
2. 数值字段不需要引号，如: edu_level > 5
...
class MilvusSearchManager:
    ...
    def search_documents(self,
        ...
        try:
            ...
            output_fields = ["job_name", "job_salary", "job_edu", "job_url", "text_for_embedding"]
...
if __name__ == "__main__":
    try:
        ...
        search_manager = MilvusSearchManager(
            ...
            db_name=os.getenv("MILVUS_DATABASE_NAME", "jobs_rag"),
        ...
        if filter_result["success"]:
            ...
            if filter_result["results"] and len(filter_result["results"]) > 0:
                filtered_items = [
                    (
                        res.entity.get("job_name", ""),
                        res.entity.get("job_salary", ""),
                        res.entity.get("job_edu", ""),
                        res.entity.get("job_url", ""),
                        res.entity.get("text_for_embedding", ""),
                        res.distance
                ...
                for idx, item in enumerate(filtered_items, 1):
                    job_name, job_salary, job_edu, job_url, text_for_embedding, distance = item
                    record = (
                        f"过滤文章{idx}:\n"
                        f"职位标题: {job_name}\n"
                        f"职位薪水: {job_salary}...\n"
